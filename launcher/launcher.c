...
static void crash_resume(void) {
    ...
    for (;;) {
        sdk_panel_led((ub_get_timer(0) / 150) & 1);  /* fast blink with "Err" */
...
static int launch(const struct entry *e) {
    ...
    }
    {
        ...
        message(msg, WHITE);
        sdk_panel_show("---");             /* apps may show their own */
    }
    ...
    sdk_libc_reset();
    sdk_panel_invalidate();                        /* the app used the panel too */
...
static void about(void) {
    ...
    for (;;) {
        if (redraw) {
            ...
            fb_text(&fb, LIST_X, 230, sdk_box_sat ?
                     "App launcher and SDK, running on the satellite box" :
                     "App launcher and SDK for the NC5874 set-top box", 2,
                     RGB(220, 225, 235), TRANSPARENT);
            fb_text(&fb, LIST_X, 264, sdk_box_sat ?
                     "Montage M88CS8002B, MIPS 24KEc ~648 MHz, 128 MB, no OS" :
                     "Montage M88CS8051B, MIPS 24KEc ~648 MHz, 128 MB, no OS",
                     2, GREY, TRANSPARENT);
            ...
            fb_text(&fb, LIST_X, 584, help[items[item]][0], 2, GREY, TRANSPARENT);
            fb_text(&fb, LIST_X, 608, help[items[item]][1], 2, GREY, TRANSPARENT);
            fb_text(&fb, LIST_X, 634, status, 2, status_col, TRANSPARENT);
            fb_rect(&fb, 0, 660, fb.w, 60, PANEL);
            fb_text(&fb, LIST_X, 680, restart ?
...
static void standby(void) {
    ...
    sdk_panel_show("");
    for (i = 0; i < 3; i++) {               /* satellite front LED: 3 blinks, then off */
        sdk_panel_led(1);
        ub_udelay(170000);
        sdk_panel_led(0);
        ub_udelay(170000);
    ...
    ub_udelay(300000);
    for (;;) {
        /* the satellite box's front has no POWER button: MENU wakes it too */
        if ((sdk_key_poll(&k) && !k.repeat &&
             (k.btn == BTN_POWER || (sdk_box_sat && k.btn == BTN_MENU))) || standby_pressed()) {
...
int main(int argc, char *argv[]) {
    ...
    crash_install();
    sdk_panel_led(0);                      /* LED: players blink it, menu off */
    sdk_panel_show("---");
    splash();
    ...
    for (;;) {
        if (redraw) {
            ...
            draw_all();
            redraw = 0;
            panel_sel = -1;                     /* after an app or a page */
        }
        if (sel != panel_sel) {                 /* front display: entry number */
            char num[8];

            snprintf(num, sizeof(num), "%3d", sel + 1);
            sdk_panel_show(count ? num : "---");
            sdk_panel_led(0);
            panel_sel = sel;
        }
        if (!sdk_key_poll(&k)) {
