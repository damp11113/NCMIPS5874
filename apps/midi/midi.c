 ...
 * YELLOW choose SoundFont, MENU front LED blink (quarter note / whole
 * note / second, saved in APPSDATA/MIDI/CONFIG.TXT), STOP / BACK list.
 * The screen shows the 16 MIDI channels with their instruments. On the
 * satellite box the front display shows the active voices ("PAU" paused,
 * volume or blink mode for 1.5 s after a change), the LED blinks with the
 * song and stays on while paused.
 */
...
/* Front LED blink (satellite box), saved as led=quarter|whole|second */
enum { BLINK_QUARTER, BLINK_WHOLE, BLINK_SECOND, BLINK_MODES };
static const char *const blink_names[BLINK_MODES] = { "quarter", "whole", "second" };
static const char *const blink_panel[BLINK_MODES] = { "b 4", "b 1", "SEc" };
...
static void draw_player(int idx) {
    ...
    fb_text(&fb, 40, 650, "OK pause  UP/DOWN prev/next  LEFT/RIGHT vol  RED/GREEN seek", 2, GREY,
             TRANSPARENT);
    fb_text(&fb, 40, 684, sdk_box_sat ?
             "BLUE engine  YELLOW SoundFont  MENU LED blink  BACK list  HOME quit" :
             "BLUE engine  YELLOW SoundFont  BACK list  HOME quit", 2, GREY, TRANSPARENT);
}
...
static void draw_status(void) {
    ...
    snprintf(line, sizeof(line), sdk_box_sat ? "Tempo %3d bpm   vol %3d%%   LED %-7s" :
              "Tempo %3d bpm   vol %3d%%", smf_tempo_bpm(), vol, blink_names[blink_mode]);
...
/* Front display (satellite box): active voices, "PAU" when paused, a note
 * (volume "U80" / "120", blink mode) for 1.5 s after a change */
static u32 panel_note_until;
static char panel_note_text[8];

static void panel_note_vol(void) {
    snprintf(panel_note_text, sizeof(panel_note_text), vol < 100 ? "U%2d" : "%3d", vol);
    panel_note_until = ub_get_timer(0) + 1500;
}

static void panel_update(void) {
    char s[8];

    if (ub_get_timer(0) < panel_note_until) {
        strcpy(s, panel_note_text);
    } else if (paused) {
        strcpy(s, "PAU");
    } else {
        snprintf(s, sizeof(s), "%3d", engine->voices());
    }
    sdk_panel_show(s);
}

/* Front LED: steady while paused, else a flash on each quarter note (first
 * quarter of the beat), each whole note (first half beat of every 4) or
 * each second (on for half a second). Written only when it changes. */
static void led_update(void) {
    ...
    if (paused) {
        on = 1;
    } else if (blink_mode == BLINK_SECOND) {
        on = smf_position_ms() % 1000 < 500;
    } else {
        b = smf_position_beats_q16();
        on = blink_mode == BLINK_QUARTER ? (b & 0xffff) < 0x4000 : b % (4u << 16) < 0x8000;
    }
    sdk_panel_led(on);
}

static void blink_next(void) {
    blink_mode = (blink_mode + 1) % BLINK_MODES;
    strcpy(panel_note_text, blink_panel[blink_mode]);
    panel_note_until = ub_get_timer(0) + 1500;
    sdk_config_set("led", blink_names[blink_mode]);
    if (sdk_config_save() < 0) {
        printf("midi: LED blink '%s' not saved (APPSDATA/MIDI/CONFIG.TXT missing?)\n",
                blink_names[blink_mode]);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    fb_clear(&fb, BG);
    if (sdk_config_load() == 0) {
        const char *led = sdk_config_get("led", blink_names[BLINK_QUARTER]);

        for (i = 0; i < BLINK_MODES && strcmp(led, blink_names[i]); i++) {
        }
        blink_mode = i < BLINK_MODES ? i : BLINK_QUARTER;
    }
